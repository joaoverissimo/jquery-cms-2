...
// <editor-fold defaultstate="collapsed" desc="Inserir, Update, Deletar">       
    public static function Inserir($Conexao, $nome, $uf, $ibge, $die = false) {
        throw new jquerycmsException("Não é possível inserir estados diretamente. Utilize o recurso de estados remoto.");
        return parent::Inserir($Conexao, $nome, $uf, $ibge, $die);
    }

    public static function Update($Conexao, $cod, $nome, $uf, $ibge, $die = false) {
        throw new jquerycmsException("Não é possível editar estados diretamente. Utilize o recurso de estados remoto.");
        return parent::Update($Conexao, $cod, $nome, $uf, $ibge, $die);
    }

    public static function Deletar($Conexao, $cod) {
        throw new jquerycmsException("Não é possível excluir estados diretamente. Utilize o recurso de estados remoto.");
...
class CtrlLocalizacao {
    ...
    protected function buildAutoFormField($label, $name, $htmlField, $span = '', $add = '') {
        ...
        $nameId = autoform2::returnInputId($name);

        $s = autoform2::FieldIn($nameId);
    ...
    public function getAutoFormField($exibeEstado = true, $exibeCidade = true, $exibeBairro = true, $exibeRua = true, $exibeNumero = true, $exibeComplemento = true) {
        ...
        if ($exibeEstado && $exibeCidade) {
            $html = autoform2::retornarSelectDb($name . "_estado", $this->estado, $validateString . " form-field-localizacao-estado", $ConexaoLoc, "locestado", "cod", "uf", "", "nome asc", "Selecione...");
            $html .= " " . autoform2::retornarSelectDb($name . "_cidade", $this->cidade, $validateString . " form-field-localizacao-cidade", $ConexaoLoc, "loccidade", "cod", "nome", new dataFilter("loccidade.estado", $this->estado), "nome asc", "Selecione...");
            ...
            $html .= " " . autoform2::retornarSelectDb($name . "_cidade", $this->cidade, $validateString, $ConexaoLoc, "loccidade", "cod", "nome", new dataFilter("loccidade.estado", $this->estado), "nome asc", "Selecione...");
        ...
        if ($exibeBairro) {
            $html = autoform2::retornarSelectDb($name . "_bairro", $this->bairro, $validateString, $ConexaoLoc, "locbairro", "cod", "nome", new dataFilter("locbairro.cidade", $this->cidade), "nome asc", "Selecione...");
        ...
        if ($exibeRua && $exibeNumero) {
            ...
            $html .= "<input type='text' placeholder='Logradouro' name='{$name}_rua' id='{$name}_rua' class='form-field-localizacao-rua $validateString form-control' value='{$this->rua}'></input> ";
            $html .= "<input type='text' placeholder='Número' name='{$name}_numero' id='{$name}_numero' class='form-field-localizacao-num $validateString form-control' value='{$this->numero}'></input>";
            $html = $this->buildAutoFormField("Logradouro, Número", $name . "_rua", $html);
            $html .= "</div>";
            $s .= $html;
        } elseif ($exibeRua && !$exibeNumero) {
            $html = "<input type='text' name='{$name}_rua' id='{$name}_rua' class='$validateString form-control' value='{$this->rua}'></input> ";
        ...
        if ($exibeComplemento) {
            $html = "<input type='text' name='{$name}_complemento' id='{$name}_complemento' class='form-control' value='{$this->complemento}'></input> ";
...
class CtrlLocalizacaoV2 extends CtrlLocalizacao {
    ...
    public function getCtrl($exibeCep = true, $exibeEstado = true, $exibeCidade = true, $exibeBairro = true, $exibeRua = true, $exibeNumero = true, $exibeComplemento = true, $exibeNaoSeiCep = false) {
        ...
        if ($exibeCep) {
            $html = "<input type='text' name='{$name}_cep' id='{$name}_cep' class='$validateString form-control' value='{$this->cep}' maxlength='8'></input> ";
            if ($exibeNaoSeiCep) {
                ...
                $('#{$name}_cep').bind('keyup change',function(){
                    ...
                    if (\$this.val().length == 8){
                        $.ajax({
                            ...
                            data = $.parseJSON(data);
                            if (data.status) {
                                $('#{$name}_rua').val(data.rua).trigger('change');
                                $('#{$name}_estado').val(data.estado).trigger('change');
                                $('#{$name}_cidade').load('/adm/locestado/ctrl/ajax.php?estado='+ data.estado +'&current=' + data.cidade, function() {
                                    {$name}_destaque();
                                    $('#{$name}_cidade').val(data.cidade).trigger('change');
                                    $('#{$name}_bairro').load('/adm/locestado/ctrl/ajax.php?cidade='+ data.cidade +'&current=' + data.bairro, function(){
                                        $('#{$name}_bairro').val(data.bairro).trigger('change');
                                    });
                                });
    ...
    public function PostRecebeValores() {
        ...
        if (isset($_POST[$name . "_cep"]) && isset($_POST[$name . "_cidade"]) && isset($_POST[$name . "_bairro"]) && isset($_POST[$name . "_rua"]) && isset($_POST[$name . "_numero"]) && isset($_POST[$name . "_complemento"])) {
            $this->cep = $_POST[$name . "_cep"] ? $_POST[$name . "_cep"] : null;
        }
