...
class SimpleImage2 {
        ...
        switch ($mimeType) {
            ...
            case 'image/webp':
                ...
                if (!function_exists('imagewebp')) {
                    throw new \Exception(
                    'WEBP support is not enabled in your version of PHP.', self::ERR_WEBP_NOT_ENABLED
                    );
                }
                imagesavealpha($this->image, true);
        ...
        for ($x = 0; $x < $w; $x++) {
            for ($y = 0; $y < $h; $y++) {
                ...
                $alphaColorXY = imagecolorallocatealpha(
                        $srcIm, ($colorXY >> 16) & 0xFF, ($colorXY >> 8) & 0xFF, $colorXY & 0xFF, $alpha
                );
  ...
  public function overlay($overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0) {
        ...
        self::imageCopyMergeAlpha(
                $this->image, $overlay->image, $x, $y, 0, 0, $overlay->getWidth(), $overlay->getHeight(), $opacity
        );
  ...
  public function resize($width, $height) {
        ...
        imagecopyresampled(
                $newImage, $this->image, 0, 0, 0, 0, $width, $height, $this->getWidth(), $this->getHeight()
        );
  ...
  public function rotate($angle, $backgroundColor = 'transparent') {
        ...
        $this->image = imagerotate(
                $this->image, -(self::keepWithin($angle, -360, 360)), $backgroundColor
        );
  ...
  public function text($text, $options) {
        ...
        if (!function_exists('imagettftext')) {
            throw new \Exception(
            'Freetype support is not enabled in your version of PHP.', self::ERR_FREETYPE_NOT_ENABLED
            );
        }
        ...
        if (is_array($options['shadow'])) {
            imagettftext(
                    $this->image, $size, $angle, $x + $options['shadow']['x'], $y + $options['shadow']['y'], $this->allocateColor($options['shadow']['color']), $fontFile, $text
            );
        }
  ...
  public function colorize($color) {
        ...
        imagefilter(
                $this->image, IMG_FILTER_COLORIZE, $color['red'], $color['green'], $color['blue'], 127 - ($color['alpha'] * 127)
        );
  ...
  private function allocateColor($color) {
        ...
        $index = imagecolorexactalpha(
                $this->image, $color['red'], $color['green'], $color['blue'], 127 - ($color['alpha'] * 127)
        );
        if ($index > -1) {
  ...
  public function extractColors($count = 5, $backgroundColor = null) {
        ...
        if (!class_exists('\League\ColorExtractor\ColorExtractor')) {
            throw new \Exception(
            'Required library \League\ColorExtractor is missing.', self::ERR_LIB_NOT_LOADED
            );
        }
