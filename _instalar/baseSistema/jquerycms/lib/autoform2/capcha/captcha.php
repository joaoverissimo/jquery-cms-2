<?php
   ...
   header("Content-type: image/jpeg"); // define o tipo do arquivo
    ...
    function captcha($largura,$altura,$tamanho_fonte,$quantidade_letras){
        ...
        $fonte = "segoeui.ttf"; //voce deve ter essa ou outra fonte de sua preferencia em sua pasta
        ...
        $palavra = substr(str_shuffle("ASDFQWERZXCV"),0,($quantidade_letras)); 
        $_SESSION["captcha_palavra"] = $palavra; // atribui para a sessao a palavra gerada
        for($i = 1; $i <= $quantidade_letras; $i++){ 
            imagettftext($imagem,$tamanho_fonte,rand(-15,15),($tamanho_fonte*$i),($tamanho_fonte + 10),$branco,$fonte,substr($palavra,($i-1),1)); // atribui as letras a imagem
        }
