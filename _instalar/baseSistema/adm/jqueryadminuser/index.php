...
//Lista os dados
$where = new dataFilter(dbJqueryadminuser::_cod, 1, dataFilter::op_different);
if ($filtraList) {
    $where->dataFilter(dbJqueryadminuser::_nome, $filtraList, dataFilter::op_likeMatches);
}

$orderBy = new dataOrder(dbJqueryadminuser::_nome, 'asc');

$paginaAtual = 0;
if (isset($_REQUEST['page']))
    $paginaAtual = $_REQUEST['page'];

$pager = new dataPager($paginaAtual, 15, $Conexao, 'jqueryadminuser', $where);

$dados = dbJqueryadminuser::ObjsListLeft($Conexao, $where, $orderBy, $pager->getLimit());
...
$pageVars = array('pageTitle' => __('table_jqueryadminuser'), 'pageAction' => "Resultado da Consulta", "nav-breadcrumbs" => array());
?><!doctype html>
